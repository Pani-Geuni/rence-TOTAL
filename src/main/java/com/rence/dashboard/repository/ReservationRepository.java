...
public interface ReservationRepository extends JpaRepository<ReservationViewEntity, Object>{
	...
	// 예약자 리스트
	@Query(nativeQuery = true, value = "select user_no, user_name, user_email, user_tel, reserve_stime, reserve_etime, backoffice_no, room_no, reserve_no"
			+ " from (select rownum as rnum, A.* from (select * from RESERVATION_VIEW where backoffice_no=?1 and ( ((reserve_stime > To_date(?2,'YYYY-MM-DD HH24:MI:SS') and reserve_stime < To_date(?3,'YYYY-MM-DD HH24:MI:SS') )or( reserve_etime > To_date(?2,'YYYY-MM-DD HH24:MI:SS') and reserve_etime < To_date(?3,'YYYY-MM-DD HH24:MI:SS')))"
			+ "or (reserve_stime <= To_date(?2,'YYYY-MM-DD HH24:MI:SS') and reserve_etime >= To_date(?3,'YYYY-MM-DD HH24:MI:SS')) ) and room_no=?4 )A) where rnum between ?5 and ?6")
