...
$(() => {
  ...
  $('.reserve_search_paging').on('click', '.paging-box.paging-num', function () {
    const backoffice_no = window.location.href.split('?backoffice_no=')[1].split('&')[0];
    const reserve_state = window.location.href.split('&reserve_state=')[1].split('&')[0];
  ...
  // 다음 페이지 리스트로 이동
  $('.reserve_search_paging').on('click', '.next-page-btn', () => {
  ...
  // 이전 페이지 리스트로 이동
  $('.reserve_search_paging').on('click', '.before-page-btn', () => {
    const start = Number($($('.reserve_search_paging').find('.paging-box.paging-num')[0]).text()) - 5;
    let last = Number($('.reserve_search_paging').find('.paging-box.paging-num:last').text()) - 5;
    ...
    const sample = $('.reserve_search_paging').find('.paging-num-wrap>.paging-box.paging-num:eq(0)').clone();
